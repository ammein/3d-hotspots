...
import GLTFInspect from '@/stories/assets/gltf-inspect.mp4'
...
<video width="1920" height="1080" controls>
    <source src={GLTFInspect} type="video/mp4"/>
</video>
...
This is also an optional if you need to have custom camera look at towards when click this specific hotspot. Useful if the model is needed to be a custom overview viewing experience. The value needs to have **3 number** in a **single array** that represents `x`, `y` and `z` position.

### `screens.detail.hotspots[n].focus.follow` (Optional)
This is also an optional if you need to want the hotspot label to follow camera. This is useful when you set the custom focus position behind the hotspot's direction, the label somehow behind the object. By setting this value to `true`, the hotspot label will follow the camera position. The value accepts **boolean value** only.