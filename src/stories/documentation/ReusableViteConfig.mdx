...
So it will transform html wherever the code matches `src` or `href` and change the link to local path from `/path/to/file` `./path/to/file`
...
This code is just a helper that the url of a 3DS Server must not start from origin. It should start from subpath (basically the whole url). Example: `resolveInsideBase('https://www.domain.com/assets/second-path', '/fonts/ttf/3DSV2-Bold.ttf');` will output into `'https://www.domain.com/assets/second-path/fonts/ttf/3DSV2-Bold.ttf'`
...
This code will beautifully replace any `url()` tag inside css from `url('/fonts/ttf/3DSV2-BoldItalic.ttf')` to `url('http://domain.com/fonts/ttf/3DSV2-BoldItalic.ttf')`. This code is using [vite-plugin-css-injected-by-js 🤯](https://www.npmjs.com/package/vite-plugin-css-injected-by-js)