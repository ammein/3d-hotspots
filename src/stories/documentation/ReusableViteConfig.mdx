...
{js_beautify.js(ViteConfig, { "brace_style": "collapse,preserve-inline" })}
</CodeBlock>
...
<Mermaid
    ...
    flowchart TB
        ...
        desktopRatioQ noDesktopRatio@-->|no| desktopRatioInput@{ shape: lean-r, label: "Adjust Custom Desktop Ratio" }
        desktopRatioInput --> mobileRatioQ@{ shape: diamond, label: "Confirm Default Mobile Ratio?" }
        desktopRatioQ yesDesktopRatio@-->|yes| mobileRatioQ
        mobileRatioQ noMobileRatio@-->|no| mobileRatioInput@{ shape: lean-r, label: "Adjust Custom Mobile Ratio" }
        mobileRatioQ yesMobileRatio@-->|yes| react
...
This code will beautifully replace any `url()` tag inside css. This code is using [vite-plugin-css-injected-by-js 🤯](https://www.npmjs.com/package/vite-plugin-css-injected-by-js). Example: 

<CodeBlock language='css'>
{`url('/fonts/ttf/3DSV2-BoldItalic.ttf') → url('http://domain.com/fonts/ttf/3DSV2-BoldItalic.ttf')`}
</CodeBlock>