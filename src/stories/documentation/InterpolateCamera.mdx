...
<Meta title="Documentation/Maintainer/Applied Mathematics/Camera Interpolation" />

<div>
    ### Basic Interpolation
    Now, this is a very basic formula for interpolation between two values with a factor of <LatexRenderer latex={`C`} as='span' displayMode={false} /> that accepts value between <LatexRenderer latex={`0...1`} as='span' displayMode={false} />:
    <LatexRenderer latex={`
        y = (1 - c) \\cdot y_0 + c \\cdot y_1
    `} />
    <Plot refresh={true} xAspectRatio={[-6, 6]} calculations={(data) => {
    ...
    ### Interpolate Camera Movement using Spherical Coordinates
    Now there is a catch in a specific use case when using linear interpolation. If the model has some big or tall buildings, when linear interpolation executes, it will go through the subjects. It is definitely not as intended. Therefore, we need to have interpolation, but in an orbit-like manner. Imagine a satellite orbiting the Earth.
    ...
    <Plot refresh={true} options={{
        ...
        height: 500,
        disableZoom: true,
    }} buttonText='Regenerate Points Position' xAspectRatio={[-6, 6]} calculations={(data) => {
    ...
    ### Manipulate Arc by making it wider/inner
    #### Adding a wide orbit bump
    To push the camera outward during the interpolation, we define:
    <LatexRenderer latex={`
    ...
    <Plot refresh={true} options={{
        ...
        height: 500,
        disableZoom: true,
    }} buttonText='Regenerate H & Points' xAspectRatio={[-6, 6]} calculations={(data) => {
        function normalizeVector(vector) {
            const magnitude = Math.sqrt(vector.x ** 2 + vector.y ** 2 + vector.z ** 2);
            if (magnitude === 0) {
                throw new Error("Cannot normalize a zero vector");
    ...
    ### Code Implementation
    You will see how the code implements in javascript with gsap interpolation as per below:
    <CodeBlock language="javascript" copyToClipboard showLineNumbers>
      {js_beautify.js(`/**
