...
<Plot className='w-full' yAspectRatio={[-10, 110.5]} xOffset={50} options={{
  ...
  annotations: [
    {
      y: 15,
      text: 'Focal Range Min'
    },
    {
      y: 35,
      text: 'Focal Range Max'
    },
    {
      y: 0.1,
      text: "CAMERA_NEAR_CLIP"
...
> Red line is `focalRange`, blue line is `linearizeDepth`. If you hover the graph above, you'll see that the derivative slope line represents (Tangent line in blue color) how the new `depth` value acts as fogginess interpolation that uses in interpolation as `t`
