...
<div>
  ...
  <Mermaid
    ...
    flowchart TB
      ...
      main[Main]
      %% Graph Start
      app[<h2>App</h2>] m1@==> load
      subgraph SScreen [<h2>Splash Scene</h2>]
        direction LR
        tbc
        p
        h[Headline]
      ...
      load m2@==> SScreen
      subgraph load [<h2>Loading Scene</h2>]
        progress
      end
      SScreen m3@==> main
      subgraph main [<h2>Main Scene</h2>]
        direction TB
      ...
      main m4@==> detail
      subgraph detail [<h2>Detail Scene</h2>]
        direction LR
        close
        t[Title]
        para[Paragraph]
      ...
      appHOC@{ shape: win-pane, label: "<h3>Context Provider:</h3>- Locale JSON\\n- Loading Progress Number\\n- Ready State\\n- TheatreJS App Project\\n- Metadata JSON" } -.- appM
      appM -.-> SScreen
      appM -.-> load
      appM -.-> main
      appM -.-> detail
      modelHOC@{ shape: win-pane, label: "<h3>Hooks State:</h3>- Mode(Wireframe/Default)\\n- HotspotID\\n- Rotation Degree\\n- Hotspot Data\\n- rotationDegree\\n- rotationSign<h3>Hooks Callback:</h3>- type['mode' | 'hotspot' | 'rotation' | 'hotspot-data' | 'rotation-sign']" } -.- modelM
      modelM -.-> 3dComponent
      modelM -.-> msComponent
      modelM -.-> ns
      modelM -.-> detail
      theatreHOC@{ shape: win-pane, label: "<h3>Parameter:</h3><code>[SheetName] : {  [ItemObjectName] : ISheet.Object }</code>\\n<h3>States:</h3>- subscribe" } -.- theatreM
      theatreM -.-> SScreen
      theatreM -.-> main
      theatreM -.-> detail
      %% Graph Ends
